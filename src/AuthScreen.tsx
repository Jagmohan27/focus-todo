...
const SOFT   = { type: 'spring', stiffness: 260, damping: 24 } as const
...
function Field({ id, label, type, value, onChange, placeholder, icon: Icon, autoComplete }: {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={id} className="text-[12px] font-semibold tracking-widest uppercase"
        style={{ color: 'rgba(29,29,31,0.45)' }}>{label}</label>
      ...
      <motion.div
        ...
        <input id={id} type={isPw && showPw ? 'text' : type} value={value}
...
export default function AuthScreen({ onAuth }: { onAuth: (user: UserType) => void }) {
  ...
  const toggle = () => { setMode(m => m === 'login' ? 'register' : 'login'); setError(''); setName(''); setEmail(''); setPassword('') }
    ...
    <div className="min-h-screen flex items-center justify-center px-4 antialiased" style={{ background: '#F5F5F7' }}>
      <div className="w-full max-w-sm">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}
          transition={{ ...SOFT, delay: 0.05 }} className="flex flex-col items-center mb-10">
          <motion.div whileHover={{ rotate: 15, scale: 1.1 }} transition={SPRING}
            className="w-14 h-14 rounded-2xl bg-[#0071E3] flex items-center justify-center shadow-lg mb-4">
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}
          ...
          className="bg-white rounded-3xl border border-gray-200/60 shadow-sm p-7">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <AnimatePresence initial={false}>
              {mode === 'register' && (
                <motion.div key="name-field" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
                  exit={{ height: 0, opacity: 0 }} transition={{ ...SOFT, opacity: { duration: 0.15 } }} className="overflow-hidden">
                  <Field id="auth-name" label="Full Name" type="text" value={name} onChange={setName}
            ...
            <Field id="auth-password" label="Password" type="password" value={password} onChange={setPassword}
              placeholder={mode === 'register' ? 'Min. 6 characters' : '••••••••'} icon={Lock}
            ...
            <AnimatePresence>
              {error && (
                <motion.div key="err" initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-[13px] tracking-tight"
                  style={{ backgroundColor: 'rgba(255,59,48,0.07)', color: '#FF3B30' }}>
            ...
            <motion.button id="auth-submit-btn" type="submit" whileTap={{ scale: 0.97 }} transition={SPRING}
              disabled={loading}
              className="mt-1 w-full flex items-center justify-center gap-2 py-3 rounded-xl text-[15px] font-semibold tracking-tight text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0071E3] focus-visible:ring-offset-2"
              style={{ background: loading ? 'rgba(0,113,227,0.55)' : 'linear-gradient(135deg,#0071E3,#34AADC)', cursor: loading ? 'wait' : 'pointer' }}>
              {loading ? (
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.35 }}
          className="text-center text-[11.5px] text-[#1D1D1F]/28 tracking-tight mt-5 px-4">
